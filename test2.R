...
}
{
    ...
    
    {
        ...
        # set path 
        realData_all <- read.csv("D:/OneDrive - zju.edu.cn/lab/git/time-science-test/data/Covid19CasesWH.csv", row.names = 1)  
    ...
    # retro <- 23
    ptime <- 1:(68+retro)
    mydate <- c(paste("Dec", (31-retro+1):31), paste("Jan", 1:31), paste("Feb", 1:29), paste("Mar", 1:8))
    pred <- states_mat[,9]
